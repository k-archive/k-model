...
Model.prototype.fn = function(name, fns) { console.log('Model.fn');
...
Model.prototype.evaluate = function() { console.log('Model.evaluate');
...
Model.prototype.start = function() { console.log('Model.start');
  var args = Array.prototype.slice.call(arguments);
...
Model.prototype.stop = function(subpath) { console.log('Model.stop');
  var path = this.path(subpath);
  this._stop(path);
};
Model.prototype._stop = function(fromPath) { console.log('Model._stop');
  this.root._fns.stop(fromPath);
};

Model.prototype.stopAll = function(subpath) { console.log('Model.stopAll');
  var segments = this._splitPath(subpath);
  this._stopAll(segments);
};
Model.prototype._stopAll = function(segments) { console.log('Model._stopAll');
...
Fns.prototype.get = function(name, inputPaths, fns, options) { console.log('Fns.get');
  fns || (fns = this.nameMap[name] || defaultFns[name]);
  var fn = new Fn(this.model, name, null, inputPaths, fns, options);
  return fn.get();
};

Fns.prototype.start = function(name, path, inputPaths, fns, options) { console.log('Fns.start');
  fns || (fns = this.nameMap[name] || defaultFns[name]);
...
Fns.prototype.stop = function(path) { console.log('Fns.stop');
  var fn = this.fromMap[path];
  delete this.fromMap[path];
  return fn;
};

Fns.prototype.toJSON = function() { console.log('Fns.toJSON');
  var out = [];
...
Fn.prototype.apply = function(fn, inputs) { console.log('Fn.apply');
...
Fn.prototype.get = function() { console.log('Fn.get');
  return this.apply(this.getFn, []);
};

Fn.prototype.set = function(value, pass) { console.log('Fn.set');
...
Fn.prototype.onInput = function(pass) { console.log('Fn.onInput');
...
Fn.prototype.onOutput = function(pass) { console.log('Fn.onOutput');
  var value = this.model._get(this.fromSegments);
  return this.set(value, pass);
};

Fn.prototype._setValue = function(model, segments, value) { console.log('Fn._setValue');
