...
Model.prototype.filter = function() { console.log('Model.filter');
...
Model.prototype.sort = function() { console.log('Model.sort');
  var args = Array.prototype.slice.call(arguments);
  var parsed = parseFilterArguments(this, args);
  return this.root._filters.add(
    parsed.path,
...
Model.prototype.removeAllFilters = function(subpath) { console.log('Model.removeAllFilters');
  var segments = this._splitPath(subpath);
  this._removeAllFilters(segments);
};
Model.prototype._removeAllFilters = function(segments) { console.log('Model._removeAllFilters');
...
Filters.prototype.add = function(path, filterFn, sortFn, inputPaths, options) { console.log('Filters.add');
  return new Filter(this, path, filterFn, sortFn, inputPaths, options);
};

Filters.prototype.toJSON = function() { console.log('Filters.toJSON');
  var out = [];
...
Filter.prototype.filter = function(fn) { console.log('Filter.filter');
...
Filter.prototype.sort = function(fn) { console.log('Filter.sort');
  if (!fn) fn = 'asc';
  if (typeof fn === 'function') {
    this.sortFn = fn;
    this.bundle = false;
...
Filter.prototype._slice = function(results, copy) { console.log('Filter._slice');
...
Filter.prototype.getInputs = function() { console.log('Filter.getInputs');
...
Filter.prototype.callFilter = function(items, key, inputs) { console.log('Filter.callFilter');
  var item = items[key];
  return (inputs) ?
...
Filter.prototype.ids = function(segments) { console.log('Filter.ids');
...
Filter.prototype.get = function() { console.log('Filter.get');
  var items = this.model._get(this.segments);
  var results = [];
  // if (Array.isArray(items)) {
  //   throw new Error('model.filter is not currently supported on arrays');
...
Filter.prototype.update = function(pass, segments) { console.log('Filter.update');
  this.idsCache = [];
  var ids = this.ids(segments);
  this.model.pass(pass, true)._setArrayDiff(this.idsSegments, ids, false);
};

Filter.prototype.ref = function(from) { console.log('Filter.ref');
...
Filter.prototype.destroy = function() { console.log('Filter.destroy');
  this.idsCache = [];
