...
Doc.prototype.path = function(segments) { console.log('Doc.path');
  var path = this.collectionName + '.' + this.id;
  if (segments && segments.lenth) path += '.' + segments.join('.');
  return path;
};

Doc.prototype._errorMessage = function(description, segments, value) { console.log('Doc._errorMessage');
