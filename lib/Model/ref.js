...
Model.prototype._canRefTo = function(value) { console.log('Model._canRefTo');
  return this.isPath(value) || (value && typeof value.ref === 'function');
};

Model.prototype.ref = function() { console.log('Model.ref');
  var from, to, options;
...
Model.prototype.removeRef = function(subpath) { console.log('Model.removeRef');
  var segments = this._splitPath(subpath);
  var fromPath = segments.join('.');
  this._removeRef(segments, fromPath);
};
Model.prototype._removeRef = function(segments, fromPath) { console.log('Model._removeRef');
  this.root._refs.remove(fromPath);
  this.root._refLists.remove(fromPath);
  this._del(segments);
};

Model.prototype.removeAllRefs = function(subpath) { console.log('Model.removeAllRefs');
  var segments = this._splitPath(subpath);
  this._removeAllRefs(segments);
};
Model.prototype._removeAllRefs = function(segments) { console.log('Model._removeAllRefs');
  this._removeMapRefs(segments, this.root._refs.fromMap);
  this._removeMapRefs(segments, this.root._refLists.fromMap);
};
Model.prototype._removeMapRefs = function(segments, map) { console.log('Model._removeMapRefs');
  for (var from in map) {
...
Model.prototype.dereference = function(subpath) { console.log('Model.dereference');
  var segments = this._splitPath(subpath);
  return this._dereference(segments).join('.');
};

Model.prototype._dereference = function(segments, forArrayMutator, ignore) { console.log('Model._dereference');
...
Refs.prototype.add = function(ref) { console.log('Refs.add');
...
Refs.prototype.remove = function(from) { console.log('Refs.remove');
...
Refs.prototype.toJSON = function() { console.log('Refs.toJSON');
  var out = [];
