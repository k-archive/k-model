...
var defaultType = require('k-sync-debug/lib/client').types.defaultType;
...
Model.prototype.query = function(collectionName, expression, options) { console.log('Model.query');
...
// stack error
Model.prototype.sanitizeQuery = function(expression) { console.log('Model.sanitizeQuery');
...
Model.prototype._initQueries = function(items, rpcClientCall) { console.log('Model._initQueries');
...
Queries.prototype.add = function(query) { console.log('Queries.add');
...
Queries.prototype.remove = function(query) { console.log('Queries.remove');
...
Queries.prototype.get = function(collectionName, expression, options) { console.log('Queries.get');
  var hash = expression? queryHash(collectionName, expression, options): collectionName;
  return this.map[hash];
};
Queries.prototype.toJSON = function() { console.log('Queries.toJSON');
  var out = [];
...
Query.prototype.destroy = function() { console.log('Query.destroy');
...
Query.prototype.dbQuery = function() { console.log('Query.dbQuery');
  if (this.isPathQuery) {
...
Query.prototype.send = function() { console.log('Query.send');
  this.shareQuery.send();
};

Query.prototype.setOptions = function(options) { console.log('Query.setOptions');
  this.options = options;
...
Query.prototype.setQuery = function(expression) { console.log('Query.setQuery');
...
Query.prototype.fetch = function(cb) { console.log('Query.fetch');
...
Query.prototype.subscribe = function(cb, rpcClientCall) { console.log('Query.subscribe');
  var self = this;
  cb = this.model.wrapCallback(cb);
...
Query.prototype._subscribeCb = function(cb) { console.log('Query._subscribeCb');
  var query = this;
...
Query.prototype._shareFetchedSubscribe = function(options, cb) { console.log('Query._shareFetchedSubscribe');
  this.model.root.connection.createFetchQuery(
    this.collectionName,
    this.dbQuery(),
    options,
...
Query.prototype._shareSubscribe = function(options, cb) { console.log('Query._shareSubscribe');
  var query = this;
...
Query.prototype._removeMapIds = function(ids) { console.log('Query._removeMapIds');
...
Query.prototype._addMapIds = function(ids) { console.log('Query._addMapIds');
  for (var i = ids.length; i--;) {
    var id = ids[i];
    this.idMap[id] = true;
  }
};
Query.prototype._diffMapIds = function(ids) { console.log('Query._diffMapIds');
  var addedIds = [];
  var removedIds = [];
  var newMap = {};
  for (var i = ids.length; i--;) {
...
Query.prototype._setExtra = function(extra) { console.log('Query._setExtra');
  if (extra === undefined) return;
  this.model._setDiffDeep(this.extraSegments, extra);
};
Query.prototype._setResults = function(results) { console.log('Query._setResults');
  var ids = resultsIds(results);
  this._setResultIds(ids);
};
Query.prototype._setResultIds = function(ids) { console.log('Query._setResultIds');
  this._diffMapIds(ids);
  this.model._setArrayDiff(this.idsSegments, ids);
};
Query.prototype._maybeUnloadDocs = function(ids) { console.log('Query._maybeUnloadDocs');
...
Query.prototype._flushSubscribeCallbacks = function(err, cb) { console.log('Query._flushSubscribeCallbacks');
  cb(err);
  var pendingCallback;
...
Query.prototype.unfetch = function(cb) { console.log('Query.unfetch');
  cb = this.model.wrapCallback(cb);
...
Query.prototype.unsubscribe = function(cb) { console.log('Query.unsubscribe');
  var query = this;
...
Query.prototype._getShareResults = function() { console.log('Query._getShareResults');
...
Query.prototype.get = function() { console.log('Query.get');
...
Query.prototype.getIds = function() { console.log('Query.getIds');
  return this.model._get(this.idsSegments) || [];
};

Query.prototype.getExtra = function() { console.log('Query.getExtra');
  return this.model._get(this.extraSegments);
};

Query.prototype.ref = function(from) { console.log('Query.ref');
...
Query.prototype.refIds = function(from) { console.log('Query.refIds');
  var idsPath = this.idsSegments.join('.');
  return this.model.root.ref(from, idsPath);
};

Query.prototype.refExtra = function(from, relPath) { console.log('Query.refExtra');
  var extraPath = this.extraSegments.join('.');
  if (relPath) extraPath += '.' + relPath;
  return this.model.root.ref(from, extraPath);
};

Query.prototype.serialize = function() { console.log('Query.serialize');
  var ids = this.getIds();
  var results = [];
...
Query.prototype._onChange = function(ids, previousIds) { console.log('Query._onChange');
